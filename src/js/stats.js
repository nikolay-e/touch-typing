...
export function endCurrentSession() {
  ...
  console.log(
    `Ended session: ${currentSession.id} - WPM: ${currentSession.wpm}, Accuracy: ${currentSession.accuracy.toFixed(1)}%`,
  );
...
function initializeBaseStatsStructure(charSet = allPossibleCharacters) {
  charSet.split("").forEach((char) => {
    ...
    if (!characterStats[char]) {
      ...
      if (
        typeof characterStats[char].confusedWith !== "object" ||
        characterStats[char].confusedWith === null
      ) {
...
function calculateMedianTime(char) {
  ...
  if (
    !charStat ||
    !charStat.allTimingSamples ||
    charStat.allTimingSamples.length === 0
  ) {
    return 0;
  }
  ...
  if (allSamples.length >= STATS_CONSTANTS.MIN_SAMPLES_FOR_OUTLIER_DETECTION) {
    const lowerBound = calculatePercentile(
      allSamples,
      STATS_CONSTANTS.OUTLIER_LOWER_PERCENTILE,
    );
    const upperBound = calculatePercentile(
      allSamples,
      STATS_CONSTANTS.OUTLIER_UPPER_PERCENTILE,
    );

    allSamples.forEach((sample) => {
      if (
        sample >= STATS_CONSTANTS.MIN_REASONABLE_TIME &&
...
function isOutlier(timeTaken, char) {
  ...
  if (
    timeTaken < STATS_CONSTANTS.MIN_REASONABLE_TIME ||
...
export function saveCharacterStatsToLocalStorage() {
  try {
    ...
    typingData.metadata.lastModified = new Date().toISOString();
    typingData.metadata.totalKeypresses = Object.values(
      typingData.statistics.characters,
    ...
    if (typingData.rawData.keystrokes.length > 1000) {
      typingData.rawData.keystrokes =
        typingData.rawData.keystrokes.slice(-1000);
    }
    ...
    if (error.name === "QuotaExceededError" || error.code === 22) {
      console.warn(
        "LocalStorage quota exceeded. Attempting to optimize data...",
      );
      try {
        // Remove old keystroke data to make room
        typingData.rawData.keystrokes =
          typingData.rawData.keystrokes.slice(-500);
        ...
        if (window.ui && window.ui.elements.resultDisplay) {
          window.ui.elements.resultDisplay.textContent =
            "Storage full! Data not saved.";
      ...
      if (importedStat.confusedWith) {
        ...
        for (const confused in importedStat.confusedWith) {
          existingStat.confusedWith[confused] =
            (existingStat.confusedWith[confused] || 0) +
            importedStat.confusedWith[confused];
        }
      }
    } else {
      // New character
...
function validateStatsStructure(stats) {
  ...
  for (const char in stats) {
    ...
    if (
      stat.allTimingSamples !== undefined &&
      !Array.isArray(stat.allTimingSamples)
    )
      return false;
    if (
      stat.outlierCount !== undefined &&
      (typeof stat.outlierCount !== "number" || stat.outlierCount < 0)
    )
      return false;
    if (
      stat.validSampleCount !== undefined &&
      (typeof stat.validSampleCount !== "number" || stat.validSampleCount < 0)
    )
      return false;
  }
...
function migrateLegacyData(legacyStats) {
  ...
  for (const char in legacyStats) {
    ...
    typingData.statistics.characters[char] = {
      ...
      allTimingSamples:
        legacyStat.timingSamples || legacyStat.allTimingSamples || [],
  ...
  try {
    ...
    if (newData) {
      ...
      if (validateImportedData(parsedData)) {
        ...
        console.warn(
          "New format data failed validation. Trying legacy format...",
        );
        loadLegacyData();
      }
    } else {
...
export function initializeCharacterStats(charSet = allPossibleCharacters) {
  ...
  console.warn(
    "initializeCharacterStats is deprecated; use loadCharacterStatsFromLocalStorage.",
  );
...
export function recordKeyPress(expectedChar, pressedChar, timeTaken) {
  ...
  if (typeof expectedChar !== "string" || typeof pressedChar !== "string") {
    console.error(
      "Invalid input to recordKeyPress: expectedChar and pressedChar must be strings",
    );
    return false;
  }
  ...
  if (!characterStats[expectedChar]) {
    console.warn(
      `Stats object missing for expected character: ${expectedChar}. Initializing.`,
    );
  ...
  characterStats[expectedChar].allTimingSamples =
    characterStats[expectedChar].allTimingSamples || [];
  ...
  if (characterStats[expectedChar].allTimingSamples.length > 150) {
    characterStats[expectedChar].allTimingSamples =
      characterStats[expectedChar].allTimingSamples.slice(-150);
  }

  // Update correctness counts (always count these regardless of timing outliers)
  totalCount++;
  currentSession.totalCount++;
  characterStats[expectedChar].total =
    (characterStats[expectedChar].total || 0) + 1;

  let correct = false;
  if (pressedChar === expectedChar) {
    correctCount++;
    currentSession.correctCount++;
    characterStats[expectedChar].correct =
      (characterStats[expectedChar].correct || 0) + 1;
    correct = true;
  } else {
    characterStats[expectedChar].incorrect =
      (characterStats[expectedChar].incorrect || 0) + 1;
  ...
  const allValidTime = Object.values(characterStats).reduce(
    (sum, charStat) => sum + (charStat.totalTime || 0),
    0,
  );
...
export function calculateWPM() {
  if (sessionStartTime > 0 && correctCount > 0) {
    ...
    if (elapsedMinutes >= TIMING_CONSTANTS.MIN_WPM_ELAPSED_TIME) {
...
export function getOutlierStats() {
  ...
  for (const char in characterStats) {
    ...
    if (outliers > 0 || allSamples > 0) {
      characterOutliers[char] = {
        ...
        outlierRate:
          allSamples > 0 ? ((outliers / allSamples) * 100).toFixed(1) : 0,
      };
    }
  }
  ...
  return {
    ...
    totalSamples,
    overallOutlierRate:
      totalSamples > 0 ? ((totalOutliers / totalSamples) * 100).toFixed(1) : 0,
    characterOutliers,
  };
}
