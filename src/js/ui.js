...
import {
  keyboardLayouts,
  standardSpecialChars,
  shiftSpecialCharsEng,
  keyMap as configKeyMap,
  TIMING_CONSTANTS,
} from "./config.js";
...
export function createModeButtons(
  ...
  const buttonConfigs = [
    ...
    { key: "n2gram", label: "2-grams" },
    { key: "n3gram", label: "3-grams" },
    { key: "n4gram", label: "4-grams" },
    { key: "n5gram", label: "5-grams" },
  ];
...
export function generateKeyboard(language) {
  ...
  layout.forEach((row) => {
    ...
    row.forEach((keySymbol) => {
      // Use 'keySymbol' to avoid conflict with 'key' event property
      const keyDiv = document.createElement("div");
      keyDiv.classList.add("key");
      // Assign data-key: lowercase for single chars, symbol itself for multi-char keys
      const dataKeyValue =
        keySymbol.length === 1 ? keySymbol.toLowerCase() : keySymbol;
      ...
      if (keySymbol.length > 1) {
        ...
        switch (keySymbol) {
          case "ShiftLeft":
            specificClass = "key-shift-left";
            break;
          case "ShiftRight":
            specificClass = "key-shift-right";
            break;
          case "ControlLeft":
          case "ControlRight":
            specificClass = "key-ctrl";
            break;
          case "AltLeft":
          case "AltRight":
            specificClass = "key-alt";
            break;
          case "MetaLeft":
          case "MetaRight":
            specificClass = "key-win";
            break;
          case "CapsLock":
            specificClass = "key-caps";
            break;
          case "Backspace":
            specificClass = "key-backspace";
            break;
          case "Tab":
            specificClass = "key-tab";
            break;
          case "Enter":
            specificClass = "key-enter";
            break;
          case "Space":
            specificClass = "key-space";
            break;
          default:
            specificClass = `key-${keySymbol.toLowerCase()}`; // Fallback
        }
...
export function highlightKey(char, keyMap = configKeyMap) {
  ...
  if (typeof char !== "string" && char !== null && char !== undefined) {
    console.warn(
      "Invalid character type provided to highlightKey:",
      typeof char,
    );
    return;
  }
  ...
  const previouslyHighlighted =
    elements.keyboardContainer.querySelectorAll(".key-highlight");
  ...
  if (
    ...
    /[a-zA-Zа-яА-Я]/.test(char)
  ) {
    requiresShift = true;
    keyToHighlight = char.toLowerCase();
  } else if (shiftSpecialCharsEng.includes(char)) {
    // Approximate check for Shift + Special Chars (Eng)
    requiresShift = true;
  ...
  const targetElement = elements.keyboardContainer.querySelector(
    `.key[data-key="${mappedKeyValue}"]`,
  );
  ...
  if (targetElement) {
    ...
    const fallbackElement = elements.keyboardContainer.querySelector(
      `.key[data-key="${keyToHighlight.toLowerCase()}"]`,
    );
  ...
  if (requiresShift) {
    const shiftKeys = elements.keyboardContainer.querySelectorAll(
...
export function pressKeyAnimation(dataKeyValue) {
  const keyElement = elements.keyboardContainer.querySelector(
    `.key[data-key="${dataKeyValue}"]`,
  );
  if (keyElement) {
    ...
    setTimeout(
      ...
      TIMING_CONSTANTS.KEY_ANIMATION_DURATION,
    );
  }
}
...
export function errorKeyAnimation(expectedChar, keyMap = configKeyMap) {
  ...
  const targetKeyElement = elements.keyboardContainer.querySelector(
    `.key[data-key="${mappedKey}"]`,
  );
  if (targetKeyElement) {
    targetKeyElement.classList.add("key-error-shake");
    setTimeout(
      () => targetKeyElement.classList.remove("key-error-shake"),
      TIMING_CONSTANTS.ERROR_SHAKE_DURATION,
    );
  }
}

/** Updates the main display area with the target character/n-gram and highlights the current key. */
export function updateDisplayAndHighlight(
  target,
  position,
  isSequence,
  keyMap,
  ...
  if (target !== null && typeof target !== "string") {
    console.error(
      "Invalid target type in updateDisplayAndHighlight:",
      typeof target,
    );
    return;
  }
 ...
 */
export function updateCoverageDisplay(
  characterCoverage,
  sessionTargetCount,
  currentCharacterSet,
) {
  try {
    ...
    if (
      ...
      elements.statsContainer.classList.contains("hidden")
    ) {
      return;
    }
    ...
    if (!coverageSection) {
      ...
      elements.statsDisplay.insertBefore(
        coverageSection,
        elements.statsDisplay.firstChild,
      );
    }
    ...
    const totalChars = Array.isArray(currentCharacterSet)
      ? currentCharacterSet.length
      : 0;
    const coveredChars = Object.keys(characterCoverage).length;
    const coveragePercent =
      totalChars > 0 ? ((coveredChars / totalChars) * 100).toFixed(1) : 0;
    ...
    if (Object.keys(sessionTargetCount).length > 0) {
      ...
      html +=
        '<div style="font-size: 0.85em; margin-top: 8px;"><strong>Most Practiced:</strong><br>';
      html += sortedChars
        .map(
          ([char, count]) =>
    ...
    const uncovered = currentCharacterSet.filter(
      (char) => !characterCoverage[char],
    );
    if (uncovered.length > 0 && uncovered.length < 20) {
      html +=
        '<div style="font-size: 0.85em; margin-top: 8px; color: orange;"><strong>Not Yet Practiced:</strong> ';
      html += uncovered
        .map((char) => escapeHTML(char === " " ? "Space" : char))
        .join(", ");
      html += "</div>";
    }
...
export function updateDetailedStatsDisplay() {
  ...
  if (stats.totalCount > 0) {
    ...
    thead.innerHTML =
      "<tr><th>Character</th><th>Attempts</th><th>Correct</th><th>Incorrect</th><th>Accuracy</th><th>Avg Time (ms)</th><th>Outliers</th><th>Most Confused With (count)</th></tr>";
    ...
    sortedChars.forEach((char) => {
      ...
      if (Object.keys(charStat.confusedWith).length > 0) {
        const confusedKey = Object.keys(charStat.confusedWith).reduce((a, b) =>
          charStat.confusedWith[a] > charStat.confusedWith[b] ? a : b,
        );
...
export function toggleStatsVisibility() {
  ...
  elements.toggleStatsButton.textContent = isHidden
    ? "Show Detailed Stats"
    : "Hide Detailed Stats";
